...
from HSPARC.algorithms.utils.util import check, init
from HSPARC.algorithms.utils.transformer_act import discrete_autoregreesive_act#mat.algorithms.utils.transformer_act
from HSPARC.algorithms.utils.transformer_act import discrete_parallel_act
from HSPARC.algorithms.utils.transformer_act import continuous_autoregreesive_act
from HSPARC.algorithms.utils.transformer_act import continuous_parallel_act
from HSPARC.algorithms.mat.algorithm.MessageGenerationNetwork import MessageGenerationNetwork
from HSPARC.algorithms.mat.algorithm.GatingNetwork import GatingNetwork
...
class MAT_comm(nn.Module):
    ...
    def __init__(self, state_dim, obs_dim, action_dim, n_agent,
        ...
        self.gating_net = GatingNetwork(n_embd).to(device) # input is obs_rep (n_embd)
        
        # EMA buffer
        self.register_buffer('ema_weights', torch.zeros(n_agent, n_agent))
        self.ema_alpha = 0.1 # EMA decay rate
    ...
    def forward(self, state, obs, action, available_actions=None, steps=0, total_step=0, group_mask=None):
        ...
        if group_mask is not None:
            relations = group_mask.unsqueeze(0).repeat(batch_size, 1, 1).to(**self.tpdv)
        elif steps > self.warmup:
            relations = self.edge_return()
        else:
            relations = self.edges
        
        elations = relations.unsqueeze(0) if len(relations.shape) == 2 else relations # Ensure shape
        if len(relations.shape) == 3: relations = relations.unsqueeze(1) # [B, 1, N, N]
        
        relations_embed = self.edges_embed(relations.long())
        relations_embed = relations_embed.squeeze(1) # Adjust depending on encoder expectation, usu [B, N, N, Embd]
        ...
        v_loc, obs_rep = self.encoder(state, obs, relations_embed, attn_mask=relations.squeeze(1), dec_agent=dec_agent)

        gate_mask, gate_logits = self.gating_net(obs_rep, hard=False) # Soft mask for training gradient
        
        if self.action_type == 'Discrete':
            action = action.long()
            action_log, entropy = discrete_parallel_act(self.decoder, obs_rep, obs, action, relations_embed, relations.squeeze(1), batch_size,
                                                        self.n_agent, self.action_dim, self.tpdv, available_actions, dec_agent=dec_agent)
        else:
            action_log, entropy = continuous_parallel_act(self.decoder, obs_rep, obs, action, relations_embed, relations.squeeze(1), batch_size,
                                                          self.n_agent, self.action_dim, self.tpdv, dec_agent=dec_agent)
        
        return action_log, v_loc, entropy, gate_logits # 返回 gate_logits

    def get_actions(self, args, state, obs, available_actions=None, deterministic=False, scoring_network=None, group_mask=None):
        self.time_step += 1 
        # state unused
        ...
        if group_mask is not None:
            relations = group_mask.to(**self.tpdv) # [N, N]
        else:
            relations = self.edge_return(exact=True)

        relations_batch = relations.unsqueeze(0).repeat(batch_size, 1, 1) # [B, N, N]
        relations_embed = self.edges_embed(relations_batch.long()) # [B, N, N, Embd]
        ...
        v_loc, obs_rep = self.encoder(state, obs, relations_embed, attn_mask=relations_batch, dec_agent=dec_agent)
        
        gate_mask, _ = self.gating_net(obs_rep, hard=True) # [B, N, 1]
        
        if self.action_type == "Discrete":
            output_action, output_action_log, current_W = discrete_autoregreesive_act(
                args, self.decoder, obs_rep, obs, relations_embed, relations_batch, batch_size,
                self.n_agent, self.action_dim, self.tpdv,
                available_actions, deterministic, dec_agent=dec_agent, time_step=self.time_step,
                scoring_network=scoring_network, 
                group_mask=relations, # [N, N] 
                gate_mask=gate_mask   #  [B, N, 1] 
            )
        else:
        ...
        if current_W is not None and len(current_W) > 0:
             with torch.no_grad():
                 current_W_tensor = torch.tensor(current_W, dtype=torch.float32, device=self.ema_weights.device)
            
                 if current_W_tensor.shape == self.ema_weights.shape:
                     self.ema_weights = self.ema_alpha * current_W_tensor + \
                                        (1 - self.ema_alpha) * self.ema_weights
                 else:
                     pass
